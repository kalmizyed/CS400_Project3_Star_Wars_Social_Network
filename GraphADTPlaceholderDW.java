...
public class GraphADTPlaceholderDW<T> implements ExtendedGraphADT<T> {
    ...
    public GraphADTPlaceholderDW() { vertices = new Hashtable<>(); }
     ...
     */
    protected Path dijkstrasShortestPath(T startData, T endData) {      
        //No vertex containing start or end
        if(!this.containsVertex(startData)||!this.containsVertex(endData)) throw new NoSuchElementException();
        
        //Start and end vertices
        Vertex start = this.vertices.get(startData);
        Vertex end = this.vertices.get(endData);
        
        //Stores current shortest paths to vertices
        HashMap<Vertex, Path> paths = new HashMap<Vertex,Path>();
        ...
        while(!queue.isEmpty()) {
          //Get current node
          Path pathToNode = queue.poll();
          Vertex node = pathToNode.end;
          
          //If there is no current shortest path for the node
          if(!paths.containsKey(node)) {
            ...
            for(Edge edge : node.edgesLeaving) {
              ...
              if(!visited.contains(target)) {
                //Add unvisited successor
                queue.add(new Path(pathToNode, edge));
              }
            }
          }
        }
        //No path from start to end
        if(!paths.containsKey(end)) throw new NoSuchElementException();
